    ...
    let nwr = reply_builder.build_kind_with(|nwr| {
        ...
        for reply_size_hint in reply_lookahead {
            ...
            match reply_size_hint {
                ...
                Some(ReplySizeHint::Async) => match args_set_iter.next() {
                    Some(args_nrd) => {
                        match dispatch_call(
                            uri.clone(),
                            args_nrd,
...
fn dispatch_call(
    ...
    match uri.next() {
        ...
        Some(6) => {
            ...
            log_trace!("Spawning /async: {:?}", spawn_r);
            Ok(())
        }
        not_defined => {
...
fn reply_size_hint(
    ...
    let not_supported = ReplySizeHint::sync(
    ...
    match uri.next() {
        ...
        Some(5) => {
            match uri.next() {
                ...
                None => match event_kind {
                    Call => {
                        ReplySizeHint::sync(SerDesSize::Sized(8 + 3), SerDesSize::Sized(8), Ok(()))
                    }
                    _ => not_supported,
                },
